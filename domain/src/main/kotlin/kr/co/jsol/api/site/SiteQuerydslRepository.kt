...
import kr.co.jsol.api.co2.QCo2Logger.co2Logger
import kr.co.jsol.api.micro.QMicro.micro
import kr.co.jsol.api.site.QSite.site
...
//    }
    ...
    fun getMicroList(condition: SearchCondition): List<SearchResponse> {
        ...
        val microTime: DateTemplate<*> =
            Expressions.dateTemplate(LocalDateTime::class.java, "DATE_FORMAT({0}, {1})", micro.regTime, "%Y %c %d %H")

        val co2Time: DateTemplate<*> =
            Expressions.dateTemplate(LocalDateTime::class.java, "DATE_FORMAT({0}, {1})", co2Logger.regTime, "%Y %c %d %H")
    ...
    private fun betweenTime(value: DateTimePath<LocalDateTime>, startTime: LocalDateTime, endTime: LocalDateTime): BooleanExpression? {
        return value.between(startTime, endTime) ?: null
    }
//    private fun isRegTimeBetween(condition: SearchCondition): BooleanExpression {
//        return micro.regTime.between(startTime, endTime)
//            .and(co2Logger.regTime.between(startTime, endTime))
//    }
}
